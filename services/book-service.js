...
function query(filterBy) {
    if (filterBy) {
        ...
        const filteredBooks = gBooks.filter(book => {
            return book.title.includes(title.toLowerCase()) && book.listPrice.amount >= minPrice && book.listPrice.amount <= maxPrice;
        })
...
function getNextPrevBook(bookId, diff) {
    const bookIdx = gBooks.findIndex(book => book.id === bookId);
    var newIdx = bookIdx + diff;
    newIdx = newIdx === gBooks.length ? 0 : newIdx;
    newIdx = newIdx === -1 ? gBooks.length - 1 : newIdx;
...
//     console.log(gBooks[nextBookIdx].id);
...
//     const bookIdx = gBooks.findIndex(book => book.id === bookId);
//     var nextBookIdx = bookIdx - 1;
//     nextBookIdx = nextBookIdx === -1 ? gBooks.length - 1 : nextBookIdx;
//     console.log(gBooks[nextBookIdx].id);
//     return gBooks[nextBookIdx].id;
//     // return Promise.resolve(gBooks[nextBookIdx].id);
// }
...
function addReview(bookId, review) {
    const bookIdx = gBooks.findIndex(book => book.id === bookId);
    if (!gBooks[bookIdx].reviews) {
        gBooks[bookIdx]['reviews'] = [review];
    } else {
...
function deleteReview(bookId, reviewId) {
    const bookIdx = gBooks.findIndex(book => book.id === bookId);
    let bookReviews = gBooks[bookIdx].reviews;
    const reviewIdx = bookReviews.findIndex(review => review.id === reviewId);
...
function addGoogleBook(googleBook) {
    ...
    const { id } = googleBook
    const { title, authors, publishedDate, description, pageCount, categories, language } = googleBook.volumeInfo
        // const { imageLinks } = googleBook.volumeInfo.smallThumbnail
    const { country, isEbook } = googleBook.saleInfo

    let googleBookConverted = {
        id,
        title,
        subtitle: '',
        authors,
    ...
    console.log(googleBookConverted);
    gBooks.push(googleBookConverted);
    _saveBooksToStorage();
